...
# after editing bin/sidecar or pi-bridge/ctrlx.ts, then relaunch CtrlX.
#
#   ./scripts/dev-install.sh            # copy ~/.ctrlx/plugins/pi  (discoverable)
...
# launch), then `ctrlx plugin list` shows `pi` (source "folder"). In Settings,
# enable it and click Install to drop the pi bridge extension into
# ~/.pi/agent/extensions/ctrlx.ts.
set -euo pipefail
