...
### Pair an iPhone, Android device, or another Mac
...
The iOS app currently requires a locally signed Xcode build. The Android companion
is built from [`android/`](android/README.md). Background push on iOS also requires
APNs credentials matching that build on the Relay. Foreground terminal access and
live state synchronization work without APNs.
...
| CtrlX for Android | Mobile Viewer, terminal input, history, and tmux session management | `android/app` |
...
Apple targets require a recent Xcode, Swift 6.3 or later, and macOS 15 or later.
The Android target requires JDK 17 and Android SDK 35.
