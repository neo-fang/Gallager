...
4. macOS sidebar 的远程 Host header 提供拖拽手柄；iOS Sessions 页在 `Edit` 模式下显示手柄。
5. iOS 原有 session 行重排继续保留；Host 只能拖到 Host header，session 只能使用行尾系统手柄。
...
- 为双端 Host header 增加相同的拖放载荷校验、视觉反馈和辅助功能移动动作。
