...
## Stage 15：iOS 终端底部输入控件

### 目标

将 iOS 终端的键盘显示/隐藏入口从导航栏和终端悬浮层移到底部安全区，
提供始终可见、易点击且不与终端内容手势冲突的输入控件。

### 实施范围

1. 新增一个轻量的共享底部键盘控件：键盘隐藏时显示 `Input`，键盘显示时
   显示 `Hide Keyboard`，使用系统按钮样式和不小于 44pt 的点击区域。
2. 单终端页通过 `safeAreaInset(edge: .bottom)` 展示控件，删除导航栏和
   隐藏导航栏场景的键盘悬浮按钮；复制按钮保持原有位置。
3. 多 pane 页由父视图只展示一个底部控件，键盘输入继续只发送到当前
   选中 pane；无选中 pane 或 host 断开时禁用。
4. 保留现有终端滚动、双击、长按、粘贴/选择及 tmux mouse mode 手势，不给终端
   内容区增加键盘触发事件。
5. 保留 Stage 14 的复制 sheet 焦点隔离；sheet 打开时不得弹出键盘，关闭后按原有
   规则恢复输入状态。
6. 不修改 relay、host、tmux 或 agent 协议，不引入自动弹出键盘或新手势。

### 验收标准

- 键盘隐藏时，底部安全区显示 `Input` 控件；点击后键盘弹出，控件随键盘上移并
  变为 `Hide Keyboard`。
- 单终端、隐藏导航栏和多 pane 页使用同一个底部交互，导航栏和终端右上角不再
  显示键盘按钮。
