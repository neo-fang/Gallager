...
2. 对“已选窗口消失且整个 session 暂时为空”增加可取消的短确认期。新快照到达时由
   SwiftUI task identity 自动取消旧确认；断线或尚未收到完整状态时不得弹回列表。
3. 用户通过 App 成功关闭 session 时显式返回列表，不再依赖下一次空快照触发隐式导航。
4. 保持窗口/面板选择命令、键盘状态、terminal stream 和 Host/Relay 协议不变，不增加
   重试队列、持久化状态或可配置延时。

### 验收标准

- 新建或切换 window 期间，即使出现一次空窗口快照，也不会立即离开终端页面；后续快照
  到达后继续显示正确窗口并保持可输入。
- 当前窗口被外部移除但同一 session 尚有窗口时，选择活动窗口或首个窗口；不会弹回列表。
- Host 断线、首次状态尚未到达时不自动返回；session 经确认确实不存在时才返回列表。
