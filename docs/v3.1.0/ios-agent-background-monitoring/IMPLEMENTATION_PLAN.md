...
6. 用真实 Agent 状态和终端输出驱动不确定总量的有限进度：
   - prompt 已提交：等待 Agent 启动；
   - `.working`：Agent 正在工作；
   - incremental terminal output：最多每 10 秒增加一个真实活动单位；
...
progress，并只在收到真实终端输出时增加活动单位。CtrlX 不定时伪造进度、
不循环续期，也不试图在没有活动 Agent 任务时维持 Relay 常驻。
